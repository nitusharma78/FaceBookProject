<div id="feed" class="mt-4">
    @forelse($posts as $post)
    <div class="card mb-4 shadow-sm mt-3" style="max-width: 800px; margin:auto;">

        <!-- Header -->
        <div class="card-header d-flex justify-content-between align-items-center bg-white border-0">
            <div class="d-flex align-items-center">
                <img src="{{ asset('uploads/profile.jpg') }}" alt="Profile" class="rounded-circle" width="40"
                    height="40">
                <div class="ms-2">
                    <strong>{{ $post->user->fname }} {{ $post->user->lname }}</strong> <br>
                    <small class="text-muted">{{ $post->created_at->format('d M \a\t h:i A') }}</small>
                </div>
            </div>

            <!-- Three Dot Dropdown -->
            <div class="dropdown">
                <a href="#" class="text-muted" id="dropdownMenu{{ $post->id }}" data-bs-toggle="dropdown"
                    aria-expanded="false">
                    <i class="bi bi-three-dots"></i>
                </a>
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenu{{ $post->id }}">
                    <li>
                        <form action="{{ route('posts.destroy', $post->id) }}" method="POST"
                            onsubmit="return confirm('Are you sure you want to delete this post?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="dropdown-item text-danger">Delete</button>
                        </form>
                    </li>
                </ul>
            </div>
        </div>

        <!-- Content -->
        <div class="card-body p-0">
            <p class="px-3">{{ $post->content }}</p>
            @if($post->image)
            <img src="{{ asset($post->image) }}" class="img-fluid" alt="Post Image"
                style="width:100%; max-height:500px; object-fit: cover;">
            @endif
        </div>

        <!-- Like & Comment Count -->
        <div class="d-flex justify-content-between text-muted small mb-2 px-3 mt-2">
            <span>
                <i class="bi bi-hand-thumbs-up-fill text-primary"></i> 10 Likes
            </span>

            <!-- Comment Toggle -->
            <span role="button" data-bs-toggle="collapse" data-bs-target="#comments-{{ $post->id }}"
                aria-expanded="false" aria-controls="comments-{{ $post->id }}">
                1 Comment
            </span>
        </div>

        <hr class="my-1">

        <!-- Footer (Like, Comment, Share) -->
        <div class="card-footer bg-white border-0">
            <div class="d-flex justify-content-around text-muted">
                <span><i class="bi bi-hand-thumbs-up"></i> Like</span>

                <!-- Comment Modal Button -->
                <button type="button" class="btn btn-outline-primary border-0" data-bs-toggle="modal"
                    data-bs-target="#commentModal{{$post->id}}" data-post-id="{{ $post->id }}">
                    <i class="bi bi-chat"></i> Comment
                </button>

                <span><i class="bi bi-share"></i> Share</span>
            </div>
        </div>

        <!-- Comment Collapse Section -->
        <div class="collapse" id="comments-{{ $post->id }}">
            <div class="px-3 pb-3">
                @forelse($post->comments as $comment)
                <div class="border p-2 mb-2 rounded bg-light">
                    <strong>{{ $comment->user->fname }} {{ $comment->user->lname }}</strong><br>
                    {{ $comment->comment }}
                </div>

                <div class="collapse" id="more-comments-{{ $post->id }}">
                    <div class="border p-2 mb-2 rounded bg-light">
                        <strong></strong><br>

                    </div>
                </div>
                @empty
                <p>No comments yet.</p>
                @endforelse



                <button class="btn btn-link p-0" data-bs-toggle="collapse"
                    data-bs-target="#more-comments-{{ $post->id }}" aria-expanded="false"
                    aria-controls="more-comments-{{ $post->id }}">
                    Show more comments
                </button>
            </div>
        </div>


        <!-- Comment Modal -->
        <div class="modal fade" id="commentModal{{$post->id}}" tabindex="-1"
            aria-labelledby="commentModalLabel{{$post->id}}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">

                    <div class="modal-header">
                        <h5 class="modal-title" id="commentModalLabel{{$post->id}}">Add a Comment</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>

                    <form action="{{ route('comment.store', ['post' => $post->id]) }}" method="POST">
                        @csrf
                        <input type="hidden" name="post_id" value="{{ $post->id }}">
                        <div class="modal-body">
                            <textarea class="form-control" rows="4" name="comment"
                                placeholder="Write your comment..."></textarea>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn btn-primary">Post Comment</button>
                        </div>
                    </form>
                </div>
            </div>
        </div>

    </div>
    @empty
    <p class="text-muted text-center">No posts available.</p>
    @endforelse
</div>